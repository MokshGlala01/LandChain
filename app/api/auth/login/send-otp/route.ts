...
async function rateLimit(key: string, limit: { max: number }) {
  ...
  await cacheSet(key, count + 1, 3600) // 1 hour window
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Look up user by mobileHash
    const user = await prisma.user.findUnique({ where: { mobileHash } })
    if (!user) {
      return NextResponse.json(
        { error: 'ACCOUNT_NOT_FOUND', message: 'No account found with this number. Please register.' },
        { status: 404 }
      )
    }
